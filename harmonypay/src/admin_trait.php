...
trait admin_trait
{
	...
	**/
	public function admin_global_settings()
	{
		...
		// Label for fieldset
		$fs->legend->label( __( 'HarmonyPay Gateway API URL', 'mycryptocheckout' ) );

		$fs->markup( 'm_gateway_api_url' )
			->p( __( 'Define your HarmonyPay Gateway API URL here.<br/> eg. <strong>http://YOUR-DOMAIN-NAME/api/v1/</strong>', 'mycryptocheckout' ) );

		$gateway_api_url = $fs->text( 'gateway_api_url' )
			->description( __( 'Your HarmonyPay Gateway API URL.', 'mycryptocheckout' ) )
			// Input label
			->label( __( 'Gateway API URL', 'mycryptocheckout' ) )
			->required()
			->size( 64, 64 )
			->trim()
